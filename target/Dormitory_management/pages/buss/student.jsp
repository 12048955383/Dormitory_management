...
<div id="dlgClass" class="easyui-dialog" title="数据" style="width: 350px; height: 450px; top: 80px;"
...
<div id="dlgClass-buttons">
    <a href="#" class="easyui-linkbutton  save" onclick="afterSelect('dgClass','dlgClass','classid','classname')"
       iconCls="icon-ok">确定</a>
    <a href="#" class="easyui-linkbutton cancel" onclick="javascript:$('#dlgClass').dialog('close')"
...
<div id="dlgDorm" class="easyui-dialog" title="数据" style="width: 350px; height: 450px; top: 80px;"
     data-options="modal:true" closed="true" buttons="#dlgDorm-buttons">
...
<div id="dlgDorm-buttons">
    <a href="#" class="easyui-linkbutton  save" onclick="afterDormSelect('dgDorm','dlgDorm','dormid','dormname')"
       iconCls="icon-ok">确定</a>
    <a href="#" class="easyui-linkbutton cancel" onclick="javascript:$('#dlgDorm').dialog('close')"
       iconCls="icon-cancel">取消</a>
</div>
        ...
        <table cellpadding="5">
            ...
            <tr>
                ...
                <td>
                    ...
                    <input class="easyui-textbox" readonly="readonly" type="text" name="classname" required="true"
                           id="classname"></input>
                    <a href="#" class="easyui-linkbutton" iconCls="icon-search"
                       onclick="openWin('dlgClass','班级')">选择</a>
                    <a href="#" class="easyui-linkbutton" iconCls="icon-redo"
                       onclick="clearText('classid','classname');">清空</a>
                </td>
            </tr>
            <tr>
                <td>宿舍:</td>
                <td>
                    ...
                    <input class="easyui-textbox" readonly="readonly" type="text" name="dormname" required="true"
                           id="dormname"></input>
                    <a href="#" class="easyui-linkbutton" iconCls="icon-search"
                       onclick="openDormWin('dlgDorm','宿舍')">选择</a>
...
<script type="text/javascript">
    ...
    function dormSearch() {
        var queryCondition = {'dormname': $('#paramDormpop').val().trim(), 'assignDorm': 'true'};
    ...
    function studentSearch() {
        var queryCondition = {
            'name': $('#paramStudent').val().trim(),
            'mobile': $('#paramMobile').val().trim(),
            'qq': $('#paramQq').val().trim(),
            'dormName': $('#paramDormName').val().trim()
        };
    ...
    //根据班级名称搜索
    function doSearch() {
        var queryCondition = {'classname': $('#paramClass').val().trim()};
    ...
    function edit() {
        ...
        if (row.length == 1) {
            ...
            $('#fm').form('clear');
            $('#birthday').datebox('setValue', changeDate(row[0].birthday).trim());
            $('#id').val(row[0].id.trim());
            $('#name').textbox('setValue', row[0].name.trim());
            $('#qq').textbox('setValue', row[0].qq.trim());
            $('#mobile').textbox('setValue', row[0].mobile.trim());
            $('#dormname').textbox('setValue', row[0].dorm.dormname.trim());
            $('#dormid').val(row[0].dorm.id);
            $('#classname').textbox('setValue', row[0].classes.classname.trim());
